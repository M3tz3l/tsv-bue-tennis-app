...
export const EVENTS_QUERY_KEY = (userId?: string) => [
  'events',
  userId ?? 'anon',
];
export const EVENT_DETAIL_QUERY_KEY = (userId?: string, eventId?: number) => [
  'event',
  userId ?? 'anon',
  eventId,
];
export const EVENT_SIGNUPS_QUERY_KEY = (userId?: string, eventId?: number) => [
  'event-signups',
  userId ?? 'anon',
  eventId,
];
...
export function useUpdateEvent(userId?: string) {
  const onSuccess = useEventInvalidation(userId);
  return useMutation<EventSummary, Error, { id: number; payload: UpdateEventRequest }>({
    mutationFn: ({ id, payload }) => BackendService.updateEvent(id, payload).then(throwOnFailure),
    onSuccess: (_data, variables) => onSuccess(variables.id),
  });
}

export function useDeleteEvent(userId?: string) {
  const onSuccess = useEventInvalidation(userId);
  return useMutation<ApiResult, Error, number>({
    mutationFn: (id) => BackendService.deleteEvent(id).then(throwOnFailure),
...
export function useCreateEventSignup(userId?: string) {
  return useSignupMutation(userId, (id, payload) => BackendService.createEventSignup(id, payload));
}

export function useUpdateEventSignup(userId?: string) {
  return useSignupMutation(userId, (id, payload) => BackendService.updateEventSignup(id, payload));
}

export function useDeleteEventSignup(userId?: string) {
  const onSuccess = useEventInvalidation(userId);
  return useMutation<ApiResult, Error, number>({
    mutationFn: (id) => BackendService.deleteEventSignup(id).then(throwOnFailure),
    onSuccess: (_data, id) => onSuccess(id),
  });
}
