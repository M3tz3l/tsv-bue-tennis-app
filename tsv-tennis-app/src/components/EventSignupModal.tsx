...
const EventSignupModal = ({ eventId, isOpen, onClose }: Props) => {
  ...
  return (
    <Dialog open={isOpen} onClose={() => !pending && onClose()} className="relative z-50">
      ...
      <div className="fixed inset-0 flex items-center justify-center p-4">
        <DialogPanel className="w-full max-w-lg rounded-lg bg-white shadow-xl">
          <div className="flex items-center justify-between border-b border-gray-200 px-6 py-4">
            ...
            <button aria-label="Schließen" onClick={onClose} disabled={pending}><XMarkIcon className="h-6 w-6 text-gray-400" /></button>
          </div>
          ...
          {data?.event && (
            <form onSubmit={submit} className="space-y-4 px-6 py-5">
              ...
              {data.event.allow_salad && <label className="block text-sm font-medium text-gray-700">Salat
                <input aria-label="Salat" type="number" min="0" step="1" value={saladCount} onChange={(e) => setSaladCount(e.target.value)} disabled={pending} className="mt-1 w-full rounded-md border border-gray-300 px-3 py-2" />
              </label>}
              {data.event.allow_cake && <label className="block text-sm font-medium text-gray-700">Kuchen
                <input aria-label="Kuchen" type="number" min="0" step="1" value={cakeCount} onChange={(e) => setCakeCount(e.target.value)} disabled={pending} className="mt-1 w-full rounded-md border border-gray-300 px-3 py-2" />
              </label>}
              ...
              <div className="flex flex-wrap justify-end gap-3">
                ...
                <button type="button" onClick={onClose} disabled={pending} className="rounded-md border border-gray-300 px-4 py-2 text-sm text-gray-700">Abbrechen</button>
