...
const EventDetails = ({ event, className = 'mt-3 space-y-1 text-sm' }: EventDetailsProps) => {
  ...
  return (
    <dl className={className}>
      ...
      {event.capacity !== null && <div><dt className="inline text-[var(--muted)]">Plätze: </dt><dd className="inline font-medium text-[var(--ink)]">{event.signup_people_count} / {event.capacity} Personen</dd></div>}
