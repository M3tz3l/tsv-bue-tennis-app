  ...
  return <>
    ...
    >
        <form id="event-form" onSubmit={submit} className={`${stackMdClass} px-6 py-5`}>
          ...
          <div className="grid gap-4 sm:grid-cols-2"><label className="block text-sm font-medium text-[var(--body)]">Startzeit<input aria-label="Startzeit" type="time" value={String(value('start_time'))} onChange={(e) => { set('start_time', e.target.value || null); if (!e.target.value) set('end_time', null); }} className={`${fieldControl} mt-1 border-[var(--hairline-strong)]`} /></label><label className="block text-sm font-medium text-[var(--body)]">Endzeit<input aria-label="Endzeit" type="time" disabled={!form.start_time} value={String(value('end_time'))} onChange={(e) => set('end_time', e.target.value || null)} className={`${fieldControl} mt-1 border-[var(--hairline-strong)] disabled:bg-[var(--hairline-soft)] disabled:text-[var(--muted-soft)]`} /></label></div>
