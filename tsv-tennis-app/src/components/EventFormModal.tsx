  ...
  return <>
    ...
    >
        <form id="event-form" onSubmit={submit} className={`${stackMdClass} px-6 py-5`}>
          <label className="block text-sm font-medium text-gray-700">Titel<input aria-label="Titel" value={String(value('title'))} onChange={(e) => set('title', e.target.value)} className={`${fieldControl} mt-1 border-gray-300`} /></label>
          <label className="block text-sm font-medium text-gray-700">Typ<select aria-label="Typ" value={form.type} onChange={(e) => set('type', e.target.value as EventType)} className={`${fieldControl} mt-1 border-gray-300`}><option value="event">Veranstaltung</option><option value="work-duty">Arbeitsdienst</option></select></label>
          <label className="block text-sm font-medium text-gray-700">Datum<input aria-label="Datum" type="date" value={String(value('event_date'))} onChange={(e) => set('event_date', e.target.value)} className={`${fieldControl} mt-1 border-gray-300`} /></label>
          <div className="grid gap-4 sm:grid-cols-2"><label className="block text-sm font-medium text-gray-700">Startzeit<input aria-label="Startzeit" type="time" value={String(value('start_time'))} onChange={(e) => set('start_time', e.target.value || null)} className={`${fieldControl} mt-1 border-gray-300`} /></label><label className="block text-sm font-medium text-gray-700">Endzeit<input aria-label="Endzeit" type="time" value={String(value('end_time'))} onChange={(e) => set('end_time', e.target.value || null)} className={`${fieldControl} mt-1 border-gray-300`} /></label></div>
          <label className="block text-sm font-medium text-gray-700">Beschreibung<textarea aria-label="Beschreibung" value={String(value('description'))} onChange={(e) => set('description', e.target.value)} className={`${fieldControl} mt-1 border-gray-300`} /></label>
          <label className="block text-sm font-medium text-gray-700">Ort<input aria-label="Ort" value={String(value('location'))} onChange={(e) => set('location', e.target.value)} className={`${fieldControl} mt-1 border-gray-300`} /></label>
          <div className="grid gap-4 sm:grid-cols-2"><label className="block text-sm font-medium text-gray-700">Anmeldeschluss<input aria-label="Anmeldeschluss" type="date" value={String(value('signup_deadline'))} onChange={(e) => set('signup_deadline', e.target.value || null)} className={`${fieldControl} mt-1 border-gray-300`} /></label><label className="block text-sm font-medium text-gray-700">Kapazität<input aria-label="Kapazität" type="number" min="1" value={form.capacity ?? ''} onChange={(e) => set('capacity', e.target.value ? Number(e.target.value) : null)} className={`${fieldControl} mt-1 border-gray-300`} /></label></div>
