  ...
  return <>
    ...
    >
        <form id="event-form" onSubmit={submit} className={`${stackMdClass} px-6 py-5`}>
          ...
          <div className="grid gap-4 sm:grid-cols-2"><label className="block text-sm font-medium text-[var(--body)]">Startzeit<input aria-label="Startzeit" type="time" value={String(value('start_time'))} onChange={(e) => set('start_time', e.target.value || null)} className={`${fieldControl} mt-1 border-[var(--hairline-strong)]`} /></label><label className="block text-sm font-medium text-[var(--body)]">Endzeit<input aria-label="Endzeit" type="time" value={String(value('end_time'))} onChange={(e) => set('end_time', e.target.value || null)} className={`${fieldControl} mt-1 border-[var(--hairline-strong)]`} /></label></div>
          <label className="block text-sm font-medium text-[var(--body)]">Beschreibung<textarea aria-label="Beschreibung" value={String(value('description'))} onChange={(e) => set('description', e.target.value)} className={`${fieldControl} mt-1 border-[var(--hairline-strong)]`} /></label>
          <label className="block text-sm font-medium text-[var(--body)]">Ort<input aria-label="Ort" value={String(value('location'))} onChange={(e) => set('location', e.target.value)} className={`${fieldControl} mt-1 border-[var(--hairline-strong)]`} /></label>
          <div className="grid gap-4 sm:grid-cols-2"><label className="block text-sm font-medium text-[var(--body)]">Anmeldeschluss<input aria-label="Anmeldeschluss" type="date" value={String(value('signup_deadline'))} onChange={(e) => set('signup_deadline', e.target.value || null)} className={`${fieldControl} mt-1 border-[var(--hairline-strong)]`} /></label><label className="block text-sm font-medium text-[var(--body)]">Kapazität<input aria-label="Kapazität" type="number" min="1" value={form.capacity ?? ''} onChange={(e) => set('capacity', e.target.value ? Number(e.target.value) : null)} className={`${fieldControl} mt-1 border-[var(--hairline-strong)]`} /></label></div>
          ...
          <fieldset><legend className="text-sm font-medium text-[var(--body)]">Status</legend><label className="mr-4"><input aria-label="Entwurf" type="radio" checked={form.status === 'draft'} onChange={() => set('status', 'draft')} /> Entwurf</label><label><input aria-label="Veröffentlicht" type="radio" checked={form.status === 'published'} onChange={() => set('status', 'published')} /> Veröffentlicht</label></fieldset>
        </form>
    </ModalShell>
