    ...
    <Dialog open={isOpen} onClose={() => { if (!pending) onClose(); }} className="relative z-50">
      ...
      <div className="fixed inset-0 flex items-center justify-center p-4"><DialogPanel className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-lg bg-white shadow-xl">
        ...
        <form onSubmit={submit} className="space-y-4 px-6 py-5">
          ...
          <div className="flex justify-end gap-3"><>{initialData && <button type="button" onClick={() => setShowDelete(true)} disabled={pending} className="mr-auto rounded-md border border-red-300 px-4 py-2 text-sm text-red-700">Löschen</button>}</><button type="button" onClick={onClose} disabled={pending} className="rounded-md border border-gray-300 px-4 py-2 text-sm">Abbrechen</button><button type="submit" disabled={pending} className="rounded-md bg-green-600 px-4 py-2 text-sm font-medium text-white">{initialData ? 'Aktualisieren' : 'Erstellen'}</button></div>
        </form>
      </DialogPanel></div>
    </Dialog>
