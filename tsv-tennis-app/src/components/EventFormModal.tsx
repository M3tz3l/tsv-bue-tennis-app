    ...
    <Dialog open={isOpen} onClose={() => { if (!pending) onClose(); }} className="relative z-50">
      ...
      <div className="fixed inset-0 flex items-center justify-center p-4"><DialogPanel className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-lg bg-white shadow-xl">
        ...
        <form onSubmit={submit} className="space-y-4 px-6 py-5">
          ...
          <div className="grid gap-4 sm:grid-cols-2"><label className="block text-sm font-medium text-gray-700">Startzeit<input aria-label="Startzeit" type="time" value={String(value('start_time'))} onChange={(e) => set('start_time', e.target.value || null)} className="mt-1 w-full min-h-[44px] rounded-md border border-gray-300 px-3 py-2" /></label><label className="block text-sm font-medium text-gray-700">Endzeit<input aria-label="Endzeit" type="time" value={String(value('end_time'))} onChange={(e) => set('end_time', e.target.value || null)} className="mt-1 w-full min-h-[44px] rounded-md border border-gray-300 px-3 py-2" /></label></div>
          <label className="block text-sm font-medium text-gray-700">Beschreibung<textarea aria-label="Beschreibung" value={String(value('description'))} onChange={(e) => set('description', e.target.value)} className="mt-1 w-full min-h-[44px] rounded-md border border-gray-300 px-3 py-2" /></label>
          <label className="block text-sm font-medium text-gray-700">Ort<input aria-label="Ort" value={String(value('location'))} onChange={(e) => set('location', e.target.value)} className="mt-1 w-full min-h-[44px] rounded-md border border-gray-300 px-3 py-2" /></label>
