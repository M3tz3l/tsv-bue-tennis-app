        ...
        <article className="relative border-t border-slate-200 py-4 pl-5 first:border-t-0 sm:pl-6">
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
                <div className="min-w-0">
                    ...
                    <Link className="mt-1 block text-base font-semibold text-slate-900 underline decoration-slate-300 underline-offset-4 hover:decoration-emerald-500" to="/dashboard/veranstaltungen">{event.title}</Link>
...
const UpcomingEventsList = ({ events, limit = 3, isLoading = false, error }: UpcomingEventsListProps) => {
    ...
    if (error) {
        return <section aria-label="Als Nächstes" className="min-h-56 rounded-lg border border-red-200 bg-red-50 p-4 text-red-700 shadow-sm sm:p-6"><h2 className="text-lg font-semibold">Als Nächstes</h2><p className="mt-4 text-sm">Veranstaltungen konnten nicht geladen werden.</p></section>;
    }
