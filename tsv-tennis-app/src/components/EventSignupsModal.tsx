...
const EventSignupsModal = ({ eventId, isOpen, onClose }: Props) => {
  ...
  if (!isOrga) return null;
    return <ModalShell isOpen={isOpen} onClose={onClose} title="Anmeldungen" widthClassName="max-w-3xl" panelClassName="max-h-[90vh] overflow-y-auto" backdropTestId="modal-backdrop" footer={null}>
      ...
      {error && <p className="p-6 text-red-600">Anmeldungen konnten nicht geladen werden</p>}
      {data && <div className={`${stackMdClass} px-6 py-5`}>
        ...
        <div className={stackMdClass}>{data.signups.map((signup) => <article key={signup.id} className="rounded-md border border-gray-200 p-4"><h3 className="font-medium">{signup.member_name ?? signup.member_id}</h3><p>{signup.people_count} Personen · {signup.salad_count} Salate · {signup.cake_count} Kuchen</p>{signup.comment && <p className="mt-1 text-sm text-gray-600">{signup.comment}</p>}</article>)}</div>
      </div>}
    </ModalShell>;
};
