use tower_governor::{GovernorLayer, key_extractor::KeyExtractor, GovernorError};
use tower_governor::governor::GovernorConfigBuilder;
use tower_http::services::{ServeDir, ServeFile};
use crate::config::Config;
use crate::utils::{filter_work_hours_for_user_by_year, calculate_total_hours, log_work_entries, round_to_2_decimals, extract_user_id_from_headers, get_required_hours_for_member};
use axum::{
    extract::{Json, Path, State},
    http::{HeaderMap, Method, StatusCode, Request, Uri},
    middleware::{self, Next},
    response::{IntoResponse, Json as ResponseJson, Response, Html},
...
async fn auth_middleware(
    ...
    if matches!(path, "/api/login" | "/api/register" | "/api/forgotPassword" | "/api/resetPassword") {
...
async fn reset_password(
    ...
    let teable_user = match teable::get_member_by_id_with_projection(
        ...
        Ok(Some(user)) => {
            info!("Found user for password reset: {} ({})", user.email, user.id);
            user
        },
        Ok(None) => {
    ...
    match state.database.get_user_by_email(&teable_user.email).await {
        Ok(Some(db_user)) => {
            info!("Found user in database, updating password for: {}", db_user.email);
            if let Err(e) = state.database.update_password(db_user.id, &payload.password).await {
        ...
        Ok(None) => {
            info!("User not found in database, creating new user for: {}", teable_user.email);
            ...
            match state.database.create_user(create_request).await {
                Ok(user_id) => {
                    info!("Created new user in database with ID: {} for email: {}", user_id, teable_user.email);
                },
                Err(e) => {
...
async fn dashboard(
    ...
    println!("🔍 Dashboard: Starting dashboard request for year: {}", year);
    ...
    let work_hours = teable::get_work_hours(&state.http_client).await
        .map_err(|e| {
    ...
    println!("🔍 Dashboard: Found {} work hours for user", user_work_hours.len());
    ...
    let family_data = if let Some(family_name) = &current_user.family_id {
        if !family_name.is_empty() {
            println!("🔍 Dashboard: Processing family data for family: {}", family_name);
            
            // Get family members using optimized query
            let family_members_response = teable::get_family_members(&state.http_client, family_name).await
                .map_err(|e| {
            ...
            println!("🔍 Dashboard: Found {} family members", family_members.len());
            ...
            for member in &family_members {
                println!("[FAMILY DEBUG] Member: {} | id: {} | family_id: {:?}", member.name(), member.id, member.family_id);
                ...
                let entries_normalized = member_work_hours.into_iter().map(|mut entry| {
            ...
            Some(FamilyData {
                ...
                members: family_members.iter().map(|m| FamilyMember {
                    id: m.id.clone(),
                    name: m.name(),
                    email: m.email.clone(),
                }).collect(),
    ...
    if let Some(ref personal) = response.personal {
        println!("  Personal data - Name: {}, Hours: {}, Required: {}", 
        ...
        for (i, entry) in personal.entries.iter().enumerate() {
            println!("    Entry {}: Date={}, Description={}, Hours={}", 
                i+1, entry.date, entry.description, entry.duration_hours);
        }
    }
    
    if let Some(ref family) = response.family {
        println!("  Family data - Name: {}, Required: {}, Completed: {}", 
            family.name, family.required, family.completed);
    }

    println!("✅ Dashboard: Sending response with {} personal hours and family data: {}", 
        total_hours, 
        if response.family.is_some() { "included" } else { "none" });
...
async fn work_hours(
    ...
    // Get current user by ID
    let current_user = teable::get_member_by_id(&state.http_client, &user_id).await
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?
        .ok_or(StatusCode::NOT_FOUND)?;

    // Get work hours only for this specific user (optimized)
    let work_hours = teable::get_work_hours_for_member(&state.http_client, &current_user.id).await
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
...
async fn get_work_hour_by_id(
    ...
    println!("🔍 Get Work Hour: Looking for work hour ID {} for user {}", work_hour_id, user_id);
    ...
    let work_hour = teable::get_work_hour_by_id(&state.http_client, &work_hour_id).await
        .map_err(|e| {
    ...
    match work_hour {
        Some(wh) => {
            ...
            if !belongs_to_user {
                println!("🚨 Get Work Hour: Work hour {} does not belong to user {}", work_hour_id, user_id);
                return Ok(ResponseJson(serde_json::json!({
                    "success": false,
            ...
            match (&wh.date, &wh.description, &wh.duration_seconds) {
                (Some(date), Some(description), Some(duration_seconds)) => {
                    ...
                    println!("✅ Get Work Hour: Found work hour {} for user {}", work_hour_id, current_user.name());
                    Ok(ResponseJson(serde_json::json!({
                        "success": true,
                        "data": {
                            "id": wh.id,
                ...
                _ => {
                    println!("🚨 Get Work Hour: Work hour {} has missing data", work_hour_id);
                    Ok(ResponseJson(serde_json::json!({
                        "success": false,
...
async fn create_work_hour(
    ...
    println!("🔍 Create Work Hour: Converting {} hours to {} seconds", payload.hours, duration_seconds);

    // Check for duplicate entry for this member and date
    let existing_hours = teable::get_work_hours_for_member(&state.http_client, &current_user.id).await
        .map_err(|e| {
            println!("🚨 Create Work Hour: Failed to fetch work hours for duplicate check: {}", e);
            StatusCode::INTERNAL_SERVER_ERROR
        })?;

    let duplicate = existing_hours.results.iter().any(|wh| {
        wh.date.as_deref() == Some(&payload.date)
    });

    if duplicate {
        println!("🚨 Create Work Hour: Duplicate entry for member {} on date {}", current_user.id, payload.date);
        return Ok(ResponseJson(serde_json::json!({
            "success": false,
    ...
    match teable::create_work_hour(
        ...
        Ok(work_hour) => {
            println!("✅ Create Work Hour: Successfully created work hour with ID: {}", work_hour.id);
            Ok(ResponseJson(serde_json::json!({
                "success": true,
...
async fn update_work_hour(
    ...
    println!("🔍 Update Work Hour: User ID: {}, Work Hour ID: {}", user_id, work_hour_id);
    ...
    match existing_work_hour {
        Some(wh) => {
            ...
            if !belongs_to_user {
                println!("🚨 Update Work Hour: Work hour {} does not belong to user {}", work_hour_id, user_id);
                return Ok(ResponseJson(serde_json::json!({
                    "success": false,
    ...
    println!("🔍 Update Work Hour: Converting {} hours to {} seconds", payload.hours, duration_seconds);
    ...
    match teable::update_work_hour(
        ...
        Ok(updated_work_hour) => {
            println!("✅ Update Work Hour: Successfully updated work hour with ID: {}", updated_work_hour.id);
            Ok(ResponseJson(serde_json::json!({
                "success": true,
